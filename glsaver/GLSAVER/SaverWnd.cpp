...
BEGIN_MESSAGE_MAP(CSaverWnd, CImageWnd)
...
BOOL CSaverWnd::Create() 
{
	...
	return CImageWnd::Create(WS_EX_TOPMOST, WS_VISIBLE|WS_POPUP, rect, NULL, 
		0, NULL);
}
...
void CSaverWnd::OnSysCommand(UINT nID, LPARAM lParam) 
{
	if ((nID == SC_SCREENSAVE) || (nID == SC_CLOSE))
		return;
	CImageWnd::OnSysCommand(nID, lParam);
}
...
BOOL CSaverWnd::OnNcActivate(BOOL bActive) 
{
	if (!bActive)
		return FALSE;
	return CImageWnd::OnNcActivate(bActive);
}
...
void CSaverWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	...
	CImageWnd::OnActivate(nState, pWndOther, bMinimized);
}
...
void CSaverWnd::OnActivateApp(BOOL bActive, DWORD hTask)
{
	if (!bActive)
		PostMessage(WM_CLOSE);
	CImageWnd::OnActivateApp(bActive, hTask);
}
...
{
	PostMessage(WM_CLOSE);
	CImageWnd::OnLButtonDown(nFlags, point);
}
